...
| :---: | :--- | :---: | :---: | :---: | :--- |
| **5.4.1** | Password ถูกต้อง | เกิด | เกิด | Passed | N/A |
| **5.4.2** | Password ผิด | เกิด | ไม่เกิด | Failed | 15 / WIFI_REASON_HANDSHAKE_TIMEOUT หรือ 204 |
...
1. เหตุใดกระบวนการ **4-Way Handshake** จึงพิสูจน์ทราบรหัสผ่าน Wi-Fi ได้โดยไม่ต้องส่งรหัสผ่าน (Passphrase) ลอยไปในอากาศเลยแม้แต่แพ็กเกจเดียว?
   * **ตอบ:** เพราะทั้งสองฝั่งใช้ความลับร่วมกันคือคีย์ PMK และนำตัวเลขสุ่มที่แลกเปลี่ยนกัน (ANonce, SNonce) ไปคำนวณคีย์ชั่วคราว (PTK) และยืนยันความถูกต้องด้วยรหัสตรวจสอบข้อมูล (MIC) โดยไม่ต้องส่งรหัสผ่านจริงข้ามเครือข่าย
...
2. อธิบายบทบาทและที่มาของคีย์ **PMK (Pairwise Master Key)** และ **PTK (Pairwise Transient Key)** ว่ามีความสัมพันธ์กันอย่างไรในการเข้ารหัสเฟรมข้อมูล?
   * **ตอบ:** 
     * **PMK:** เป็นคีย์หลักที่แปลงมาจากรหัสผ่านและชื่อ SSID
     * **PTK:** เกิดจากการนำ PMK มาร่วมคำนวณกับตัวสุ่ม (Nonces) และ MAC Address ของทั้งสองฝั่ง และนำ PTK ไปใช้ในการเข้ารหัสข้อมูลจริง
...
3. เหตุใดเมื่อเราพิมพ์ Password ผิด (ข้อ 5.4.2) ESP32 จึงยังคงได้รับ Event **`WIFI_EVENT_STA_CONNECTED`** ก่อนที่จะเกิด Event **`WIFI_EVENT_STA_DISCONNECTED`** ตามมาในภายหลัง?
   * **ตอบ:** เพราะเฟส 2-3 (Auth & Assoc) ที่ทำให้เกิด Event `CONNECTED` นั้นทำงานเสร็จก่อนที่จะเข้าสู่ขั้นตอน 4-Way Handshake ในเฟส 4 เมื่อตรวจสอบพบรหัสผ่านผิดในเฟส 4 จึงมีคำสั่งตัดการเชื่อมต่อและเกิด Event `DISCONNECTED` ตามมา
...
4. หากเครือข่าย Wi-Fi ไม่มี DHCP Server (ไม่มีการแจก IP อัตโนมัติ) ผลการทดลองในข้อ 5.4.1 จะหยุดอยู่ที่ขั้นตอนใด และจะไม่เกิด Event ใดขึ้น?
   * **ตอบ:** จะหยุดค้างหลังจากสถาปนาคีย์เสร็จ (เฟส 4 สำเร็จ) ในขั้นตอนรอรับ IP (เฟส 5) และจะไม่เกิด Event `IP_EVENT_STA_GOT_IP` ขึ้นเลย

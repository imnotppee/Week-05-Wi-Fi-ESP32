...
| :---: | :--- | :---: | :---: | :---: | :--- |
| **5.2.1** | SSID และ Password ถูกต้อง | `IP_EVENT_STA_GOT_IP` | Passed | N/A | เชื่อมต่อสำเร็จและได้รับ IP Address |
| **5.2.2** | ระบุ SSID ผิด (ไม่มีในระบบ) | `WIFI_EVENT_STA_DISCONNECTED` | Failed | 201 / 0xC9 | WIFI_REASON_NO_AP_FOUND (ไม่พบสัญญาณ Wi-Fi) |
| **5.2.3** | ระบุ SSID ถูกต้อง แต่ Password ผิด | `WIFI_EVENT_STA_DISCONNECTED` | Failed | 15 / 0x0F หรือ 204 / 0xCC | WIFI_REASON_HANDSHAKE_TIMEOUT / 4WAY_HANDSHAKE_TIMEOUT (รหัสผ่านผิด) |
...
1. เหตุใดการระบุ SSID ผิด (ข้อ 5.2.2) จึงส่งผลให้เกิด Disconnect Event ด้วย Reason Code `201` (`WIFI_REASON_NO_AP_FOUND`) ตั้งแต่เฟส Scan?
   * **ตอบ:** เพราะก่อนเชื่อมต่อ บอร์ดต้องสแกนหาเครือข่ายก่อน เมื่อสแกนแล้วไม่พบชื่อ SSID ตามที่ระบุในพื้นที่ จึงส่ง Event การเชื่อมต่อล้มเหลวด้วยรหัส 201 (หา AP ไม่เจอ) ทันที
...
2. เหตุใดการพิมพ์ Password ผิด (ข้อ 5.2.3) จึงผ่านเฟส Auth และ Assoc มาได้ แต่มาล้มเหลวในเฟส 4-Way Handshake (Reason Code `15` หรือ `204`)?
   * **ตอบ:** เพราะขั้นตอน Auth และ Assoc เป็นการเชื่อมต่อตกลงสัญญาณวิทยุเบื้องต้น (ระดับ L2 Link-Layer) โดยยังไม่เช็ครหัสผ่าน รหัสผ่านจริงจะถูกตรวจสอบในเฟส 4-Way Handshake (EAPOL) เมื่อรหัสผิดการถอดรหัสจึงล้มเหลวและถูกตัดการเชื่อมต่อ
...
3. ลำดับการเกิด Event ระหว่าง **`WIFI_EVENT_STA_CONNECTED`** กับ **`IP_EVENT_STA_GOT_IP`** Event ใดเกิดขึ้นก่อนกัน และมีความหมายทางกายภาพของ Layer Network ต่างกันอย่างไร?
   * **ตอบ:** 
     * `WIFI_EVENT_STA_CONNECTED` เกิดก่อน: หมายถึงบอร์ดจับคู่สัญญาณวิทยุกับเสาสัญญาณเราเตอร์สำเร็จแล้ว (ระดับ Link Layer)
     * `IP_EVENT_STA_GOT_IP` เกิดทีหลัง: หมายถึงบอร์ดส่งรหัสผ่านผ่านและได้รับหมายเลขไอพีแอดเดรสสำหรับใช้ส่งข้อมูลอินเทอร์เน็ตแล้ว (ระดับ Network Layer)
...
4. สมาชิกตัวแปร `reason` ในโครงสร้าง `wifi_event_sta_disconnected_t` มีประโยชน์อย่างไรต่อการออกแบบระบบค้นหาสาเหตุและกู้คืนการเชื่อมต่อ (Auto-Reconnection Mechanism) ในแอปพลิเคชัน IoT?
   * **ตอบ:** ช่วยให้บอร์ดแยกแยะสาเหตุและจัดการต่อใหม่ได้เหมาะสม เช่น หากได้รหัส `201` (หาไม่เจอ) ให้เว้นระยะเวลาแล้วสแกนหาใหม่ แต่ถ้าได้รหัส `15` หรือ `204` (รหัสผิด) ให้หยุดเชื่อมต่อแล้วเตือนผู้ใช้แก้รหัสผ่าน เป็นต้น

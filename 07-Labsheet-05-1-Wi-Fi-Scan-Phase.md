...
| :---: | :--- | :---: | :---: | :---: |
| **5.1.1** | สแกนทั่วไปทุก Channel | Success (ESP_OK / 0x0) | 19 | ~2250 ms |
| **5.1.2** | กำหนดสแกนเฉพาะ Channel 1 | Success (ESP_OK / 0x0) | 0 | ~150 ms |
| **5.1.3** | กำหนดสแกน SSID ที่มีจริง | Success (ESP_OK / 0x0) | 1 | ~2230 ms |
| **5.1.4** | กำหนดสแกน SSID ที่ไม่มีจริง | Success (ESP_OK / 0x0) | 0 | ~2240 ms |
...
| 5 | <Hidden SSID> | 64:20:E3:E7:30:18 | -64 dBm | 3 | WPA_WPA2_PSK |
...
1. การกำหนดค่าในโครงสร้าง `wifi_scan_config_t` สำหรับสแกนเจาะจงเฉพาะช่องความถี่ (ข้อ 5.1.2) ช่วยลดเวลาในการสแกนเมื่อเทียบกับการสแกนทุกช่องความถี่ (ข้อ 5.1.1) อย่างไร และมีข้อจำกัดอย่างไร?
   * **ตอบ:** 
     * **ช่วยลดเวลา:** สแกนเพียงช่องเดียว ไม่ต้องสลับช่อง 1-13 (ลดจาก ~2 วินาที เหลือ ~0.15 วินาที)
     * **ข้อจำกัด:** จะมองไม่เห็นสัญญาณ Wi-Fi ที่ปล่อยอยู่ในช่องสัญญาณอื่นเลย
...
2. เมื่อสังเกตผล Forensic Log ในข้อ 5.1.4 (สแกนหา SSID ที่ไม่มีอยู่จริง) ฟังก์ชัน `esp_wifi_scan_start()`, `esp_wifi_scan_get_ap_num()` และ `esp_wifi_scan_get_ap_records()` ส่งคืนค่าอย่างไร?
   * **ตอบ:**
     * `esp_wifi_scan_start()` ส่งคืน `ESP_OK` (สั่งรันสแกนสำเร็จ)
     * `esp_wifi_scan_get_ap_num()` ส่งคืน `ESP_OK` และได้จำนวน AP = 0
     * `esp_wifi_scan_get_ap_records()` ส่งคืน `ESP_OK` แต่ไม่ได้ดึงข้อมูลอะไรกลับมา
...
3. ค่าระดับความแรงสัญญาณ (RSSI) ที่แสดงเป็นตัวเลขติดลบ (เช่น -45 dBm กับ -80 dBm) ค่าใดแสดงถึงสัญญาณที่มีความแรงและความเสถียรมากกว่ากัน?
   * **ตอบ:** ค่า `-45 dBm` แรงและเสถียรกว่า เพราะมีค่าติดลบน้อยกว่า (เข้าใกล้ 0 มากกว่า) แปลว่ากำลังสัญญาณวิทยุเข้มข้นกว่าและตัวรับอยู่ใกล้เราเตอร์มากกว่า
...
4. เหตุใดการดึงค่า `authmode` (`wifi_auth_mode_t`) จากโครงสร้าง `wifi_ap_record_t` จึงมีความสำคัญต่อการเตรียมการในเฟสถัดไป (Authentication & Association Phase)?
   * **ตอบ:** เพื่อจะได้รู้ว่าเราเตอร์ใช้วิธีล็อกรหัสความปลอดภัยแบบใด (เช่น WPA2, WPA3 หรือไม่ได้ล็อก) จะได้เลือกกำหนดค่ารหัสผ่านและความปลอดภัยฝั่ง ESP32 ให้ตรงกัน ทำให้การเชื่อมต่อไม่ล้มเหลว